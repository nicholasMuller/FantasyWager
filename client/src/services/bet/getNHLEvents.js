...
export const getNHLEvents = async () => {
  const events = await getWeeklyGameIds();
  let matchups = [];
  for (const eventId of events) {
    let eventResponse = await axios.get(
      `http://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/events/${eventId}?lang=en&region=us`
    );
    eventResponse = eventResponse.data;
    let oddsResponse = await axios.get(
      `http://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/events/${eventId}/competitions/${eventId}/odds?lang=en&region=us`
    );
...
const getWeeklyGameIds = async () => {
  ...
  let response = await axios.get(
    `http://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/events?lang=en&region=us`
  );
